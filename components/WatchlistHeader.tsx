...
export function WatchlistHeader({ watchlist, collaborators = [], sortOption, onSortChange }: WatchlistHeaderProps) {
    return (
        <View className="bg-surface pb-4 border-b border-white/10">
            <View className="p-6 items-center">
                <Text className="text-3xl font-bold text-text-primary text-center mb-2">{watchlist.name}</Text>
                ...
                {watchlist.isShared && watchlist.shareCode && (
                    <View className="mt-2 bg-blue-900/30 px-3 py-1 rounded-full border border-blue-500/30">
                        <Text className="text-blue-300 text-xs font-mono tracking-widest">CODE: {watchlist.shareCode}</Text>
                    </View>
                )}
            </View>
            ...
            <View className="px-4">
                <View className="flex-row bg-black/20 p-1 rounded-lg">
                    ...
                    >
                        <Text className={`text-xs ${sortOption === 'release_date' ? 'text-white font-semibold' : 'text-text-secondary'}`}>Release Date</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        className={`flex-1 py-1.5 rounded-md items-center ${sortOption === 'added_latest' ? 'bg-white/10' : ''}`}
                        onPress={() => onSortChange('added_latest')}
                    >
                        <Text className={`text-xs ${sortOption === 'added_latest' ? 'text-white font-semibold' : 'text-text-secondary'}`}>Latest Added</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        className={`flex-1 py-1.5 rounded-md items-center ${sortOption === 'added_oldest' ? 'bg-white/10' : ''}`}
                        onPress={() => onSortChange('added_oldest')}
                    >
                        <Text className={`text-xs ${sortOption === 'added_oldest' ? 'text-white font-semibold' : 'text-text-secondary'}`}>Oldest Added</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
