...
export default function ImportPage() {
  ...
  async function handleFile(file: File | null) {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Could not parse that file.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleImport() {
    ...
    try {
      for (let index = 0; index < included.length; index += 1) {
        ...
        try {
          const resolvedTitle = await resolveTitleId(
            item,
            (id, newMediaType) => {
          ...
          const existingDiaryEntry = await db.getLastWatchEntry(
          ...
          if (existingDiaryEntry) {
            ...
            if (item.rating === null)
              throw new Error(`"${item.title}" needs a rating.`);
            ...
            for (const episode of episodes) {
              ...
              if (rating === null)
                throw new Error(`"${item.title}" needs ratings before import.`);
          ...
          console.error(
            `[Import] Web item import failed for: ${item.title}`,
            caught,
          );
          const errorMsg =
      ...
      setError(
        failureCount > 0 ? `${failureCount} item(s) failed to import.` : null,
      );
    } catch (caught) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(state.items.length / ITEMS_PER_PAGE),
  );
      ...
      <Card className="mb-6 grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <h2 className="text-lg font-semibold text-kino-text">
            TV Time ZIP or Letterboxd CSV
          </h2>
          <p className="mt-2 text-sm leading-6 text-kino-muted">
            Choose an export file. You can edit titles, dates, ratings, and
            inclusion before saving.
          </p>
          {state.fileName ? (
            <p className="mt-2 text-sm font-semibold text-kino-accent">
              {state.fileName}
            </p>
          ) : null}
        </div>
        ...
        <label className="flex items-center gap-2 text-sm font-semibold text-kino-muted">
          {item.importStatus === "success" ? (
            ...
            <input
              ...
              onChange={(event) =>
                onChange(item.id, { include: event.target.checked })
              }
              type="checkbox"
            />
          )}
...
async function resolveTitleId(
  ...
  const searchAndMatch = async (
    ...
    try {
      ...
      let chosen = chooseBestSearchCandidate(
        item.title,
        item.year,
        primaryCandidates,
      );
