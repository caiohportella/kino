...
export default function DiaryPage() {
  ...
  const diaryCounts = useMemo(() => {
    ...
    for (const entry of entries)
      counts.set(entry.titleId, (counts.get(entry.titleId) || 0) + 1);
    return counts;
  }, [entries]);
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredEntries.length / DIARY_ITEMS_PER_PAGE),
  );
  ...
  const years = useMemo(
    () =>
      Array.from(
        new Set(
          entries.map((entry) =>
            String(new Date(entry.watchedAt).getFullYear()),
          ),
        ),
  ...
  return (
    <div className="content-frame">
      ...
      {entries.length === 0 ? (
        ...
        <EmptyState
          action={
            <Button onClick={resetFilters}>{t("diaryFilters.reset")}</Button>
          }
        ...
        <div className="grid gap-8">
          {sections.map((section) => (
            <section key={section.title}>
              <h2 className="mb-3 text-base font-semibold text-kino-muted">
                {section.title}
              </h2>
...
function filterAndSortDiaryEntries(
  ...
  const filtered = entries.filter((entry) => {
    ...
    if (filters.watchType !== "any" && entry.watchType !== filters.watchType)
      return false;
    if (
      filters.year !== "any" &&
...
function DiaryRow({
  ...
  const localized =
    localizedTitles[
      localizedTitleKey({ tmdbId: entry.tmdbId, type: entry.type })
    ];
  const displayTitle = localized?.title || entry.titleName;
  const poster = getTmdb().getImageUrl(
    localized?.posterPath ?? entry.coverImage,
    "w200",
  );
    ...
    <Card className="grid grid-cols-[40px_44px_minmax(0,1fr)_auto] items-center gap-3 p-3 lg:grid-cols-[48px_56px_minmax(0,1fr)_160px_auto] lg:gap-4 lg:p-4">
      ...
      <div className="min-w-0">
        <Link
          ...
          href={titlePath(entry.tmdbId, entry.titleName, entry.type)}
        >
          <h3 className="truncate font-semibold text-kino-text">
            {displayTitle}
          </h3>
        </Link>
            ...
            <PopoverContent align="end" className="w-auto p-0">
              <Calendar
                ...
                onSelect={(date) => {
                  if (!date) return;
                  setDateOpen(false);
                  void onUpdate(entry, { watchedAt: date }).catch(
                    () => undefined,
                  );
                }}
...
function DiaryDialog({
  ...
  const [watchedAt, setWatchedAt] = useState(
    () => new Date(entry?.watchedAt || Date.now()),
  );
  const [watchType, setWatchType] = useState<WatchType>(
    entry?.watchType || "first-time",
  );
