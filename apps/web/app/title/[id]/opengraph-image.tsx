...
export default async function OpenGraphImage({
  ...
  if (!Number.isFinite(tmdbId) || tmdbId <= 0) {
    return new ImageResponse(<FallbackBanner title="Title not found" subtitle="Kino" />, { ...size })
  }
  ...
  try {
            ...
            <div style={{ display: 'flex', flex: 1, flexDirection: 'column', justifyContent: 'space-between', gap: 42 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={subtleLabel}>{type === 'tv' ? 'TV series' : 'Movie'} preview</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22, maxWidth: 640 }}>
                ...
                <div style={chipRow}>
                  {(type === 'tv'
                    ? [
                        details.totalSeasons ? `${details.totalSeasons} seasons` : 'Series',
                        details.runtime ? `${details.runtime} min` : 'Episode detail',
                        'Watchlist-ready',
                      ]
                    : [details.runtime ? `${details.runtime} min` : 'Feature film', 'Diary-ready', 'Ratings']
                  ).map((chip) => (
            ...
            <div style={{ display: 'flex', width: 390, alignItems: 'center', justifyContent: 'center' }}>
              <div style={posterFrame}>
                ...
                <div style={posterTitleStack}>
                  <div style={posterMiniLabel}>{type === 'tv' ? 'Series' : 'Film'}</div>
    ...
    return new ImageResponse(<FallbackBanner title="Kino" subtitle="Movies and series" />, { ...size })
  }
}

function FallbackBanner({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div style={{ ...container, padding: 72, justifyContent: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 860 }}>
        ...
        <div style={{ fontSize: 30, lineHeight: 1.35, color: 'rgba(255,255,255,0.78)' }}>{subtitle}</div>
      </div>
    </div>
  )
}
