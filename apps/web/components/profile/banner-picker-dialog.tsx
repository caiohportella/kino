    ...
    onError: (mutationError) => {
      setError(
        mutationError instanceof Error
          ...
          : t('modals.bannerPicker.searchFailed')
      )
    },
  })
  ...
  const imagesMutation = useMutation({
    ...
    onError: (mutationError) => {
      setError(
        mutationError instanceof Error
          ? mutationError.message
          : t('modals.bannerPicker.imagesFailed')
      )
    },
  })
      ...
      <DialogContent className="min-w-0 w-[calc(100%-2rem)] max-w-3xl">
        ...
        <div className="grid min-w-0 gap-4">
          {selectedMedia ? (
            ...
            >
              <label className="sr-only" htmlFor="banner-search">
                {t('modals.bannerPicker.searchLabel')}
              </label>
              <input
                autoFocus
                ...
                placeholder={t('modals.bannerPicker.searchPlaceholder')}
                value={query}
              />
            ...
            <p className="text-sm text-red-300">
              {error ||
                (saveMutation.error instanceof Error
                  ...
                  : t('modals.bannerPicker.updateFailed'))}
            </p>
          ) : null}
          ...
          <div className="flex flex-wrap justify-between gap-3">
            ...
            >
              <Trash2 size={16} />
              {t('modals.bannerPicker.removeBanner')}
            </Button>
            <Button
              disabled={saveMutation.isPending}
