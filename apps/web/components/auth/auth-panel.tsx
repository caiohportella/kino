          ...
          <TabsContent value="sign-in">
            <form className="grid gap-4" onSubmit={handleLogin}>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
                ...
                <button
                  className="font-semibold text-kino-accent hover:text-kino-accent-strong focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-kino-accent"
                  onClick={() => {
          ...
          <TabsContent value="register">
            <form className="grid gap-4" onSubmit={handleRegister}>
              ...
              <AuthField
                ...
                error={
                  submitted && !registerPasswordValid ? 'Complete all password requirements.' : null
                }
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold text-kino-text" htmlFor={id}>
      ...
      <input
        ...
        className={cn(
          'min-h-11 rounded-md border bg-white/4 px-3 text-base text-kino-text outline-none transition-colors placeholder:text-kino-muted/60 focus:border-kino-accent',
...
function StatusMessage({ error, message }: { error: string | null; message: string | null }) {
  ...
  return (
    <p
      className={cn('text-sm font-semibold', error ? 'text-red-300' : 'text-kino-accent')}
      role="status"
    >
      {error || message}
    </p>
