      ...
      <div className="grid w-full grid-cols-1 gap-3 min-[390px]:grid-cols-2 sm:flex sm:w-auto sm:max-w-full sm:flex-wrap sm:justify-center lg:justify-start">
        ...
        {hasLastWatch ? (
          ...
          >
            <SplitButtonMain
              disabled={
                (Boolean(userId) && title.id === ANON_TITLE_ID) ||
    ...
    <>
      ...
      >
        <DialogContent>
          ...
          {query.isLoading ? (
            <MediaModalSkeleton label={t("common.loading")} />
          ) : null}

          <div className="grid gap-3">
            {(query.data?.watchlists || []).map((watchlist) => {
              const contributorId =
                query.data?.selected.get(watchlist.id);
              ...
              return (
                <button
                  ...
                  title={
                    active && !canRemove
                      ? t("watchlists.onlyContributorCanRemove")
                      : undefined
                  }
                  type="button"
                >
          ...
          {!query.isLoading &&
          query.data?.watchlists.length === 0 ? (
