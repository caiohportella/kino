...
import {
  getUpcomingSeason,
  TitleHeader,
} from "@/components/title/title-header";
import {
...
export function TitlePage() {
  ...
  const reviewAuthor = currentProfileQuery.data
    ? toReviewAuthor(currentProfileQuery.data)
    : null;
  ...
  const currentUserRating =
    personalRating?.userId === user?.id
      ? Number(personalRating?.rating ?? 0)
      : 0;
            ...
            <>
              ...
              <TitleDiscoverySection
                ...
                reviewAuthorLoading={Boolean(
                  user && currentProfileQuery.isLoading,
                )}
                title={title}
                user={user}
              />
