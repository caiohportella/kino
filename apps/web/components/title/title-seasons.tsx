...
function episodeRatingKey(
  rating: Pick<EpisodeRating, "seasonNumber" | "episodeNumber">,
) {
  return `${rating.seasonNumber}:${rating.episodeNumber}`;
}

function mergeEpisodeRatings(
  current: EpisodeRating[],
  updates: EpisodeRating[],
) {
  const merged = new Map(
    current.map((rating) => [episodeRatingKey(rating), rating]),
  );
...
export function SeasonTabs({
  ...
  const seasons = useMemo(() => {
    const fromTmdb = title.seasons?.filter(
  ...
  const defaultSeason = useMemo(() => {
    ...
    for (const rating of titleRatingsQuery.data || []) {
      watchedBySeason.set(
        rating.seasonNumber,
        (watchedBySeason.get(rating.seasonNumber) || 0) + 1,
      );
    }

    const totalExpectedEpisodes = seasons.reduce(
  ...
  function handleRatingsChanged(
    savedRatings: EpisodeRating[],
        ...
        onEpisodeRemoved={(seasonNumber, episodeNumber) => {
          queryClient.setQueryData<EpisodeRating[]>(
            ...
            (current = []) =>
              current.filter(
                (rating) =>
                  rating.seasonNumber !== seasonNumber ||
                  rating.episodeNumber !== episodeNumber,
              ),
          );
          queryClient.invalidateQueries({ queryKey: titleRatingsKey });
        }}
...
function SeasonEpisodes({
  ...
  const watchableEpisodes = episodes.filter(
    (episode) => !isUnairedEpisode(episode),
  );
  ...
  function syncSavedRatings(savedRatings: EpisodeRating[]) {
    const mergedRatings = mergeEpisodeRatings(
      ratingsQuery.data || [],
      savedRatings,
    );
    queryClient.setQueryData(ratingsKey, mergedRatings);

    const mergedEpisodeNumbers = new Set(
      mergedRatings.map((rating) => rating.episodeNumber),
    );
    const completedSeason =
      !fullSeasonWatched &&
      watchableEpisodes.length > 0 &&
      watchableEpisodes.every((episode) =>
        mergedEpisodeNumbers.has(episode.episode_number),
      );
  ...
  if (seasonQuery.isLoading)
    return <MediaModalSkeleton label={t("common.loading")} />;
    ...
    <section className="mt-5 grid gap-4">
      <div className="rounded-md border border-white/10 bg-white/3 p-4">
        ...
        <div className="mt-4">
          <ProgressBar
            value={
              watchableEpisodes.length
                ? (watchedCount / watchableEpisodes.length) * 100
                : 0
            }
          />
        </div>
              ...
              >
                ...
                <div className="min-w-0">
                  <h4 className="font-semibold text-kino-text">
                    ...
                    {episode.name ||
                      t("seasons.episode", { number: episode.episode_number })}
                  </h4>
                  {episode.overview ? (
                    <p className="mt-1 line-clamp-2 text-sm text-kino-muted">
                      {episode.overview}
                    </p>
                  ) : null}
                  {episode.air_date ? (
                  ...
                  {existingRating ? (
                    <p className="mt-2 flex items-center gap-1.5 text-xs text-kino-muted">
                      ...
                      <span>
                        {t("seasons.watchedOn")}{" "}
                        {formatKinoDate(existingRating.watchedAt)}
                      </span>
                    </p>
                  ) : null}
                ...
                <div className="flex items-center gap-2 md:justify-end">
                  ...
                  {isWatched ? (
                    ...
                    <Tooltip>
                      ...
                      ></TooltipTrigger>
                      <TooltipContent>
                        {userCanRate ? watchLabel : t("auth.signIn")}
                      </TooltipContent>
                    </Tooltip>
                  )}
                </div>
      ...
      <EpisodeActionDialog
        ...
        existingRating={
          selectedEpisode
            ? ratings.get(selectedEpisode.episode_number)
            : undefined
        }
  ...
  return (
    <div className="flex max-w-xl flex-col gap-2 rounded-md border border-white/10 bg-black/20 px-3 py-2.5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {averageRating !== null ? (
          <span className="text-sm font-bold text-kino-text">
            {averageRating.toFixed(1)} / 5
          </span>
        ) : (
          <span className="text-sm font-semibold text-kino-text">
            {t("title.noRatingsYet")}
          </span>
        )}
      </div>
      {averageRating !== null ? (
      ...
      <DialogContent>
        ...
        <div className="grid gap-5">
          <section className="grid gap-2">
            <div className="text-sm font-semibold text-kino-text">
              {t("title.rateMovie")}
            </div>
            <RatingStars
          ...
          <label className="flex items-center justify-between gap-4 rounded-md border border-white/10 bg-white/4 px-4 py-3 text-sm text-kino-text">
            ...
            <input
              ...
              disabled={mutation.isPending}
              onChange={(event) =>
                setWatchType(event.target.checked ? "rewatch" : "first-time")
              }
              type="checkbox"
            />
          </label>
      ...
      <DialogContent>
        ...
        <div className="grid gap-5">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              disabled={
                mutation.isPending || rating <= 0 || episodes.length === 0
              }
              onClick={() => mutation.mutate()}
            >
              <Star size={16} />
