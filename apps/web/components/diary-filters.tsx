...
export function DiaryFilters({
  ...
  const sortOptions = SORT_GROUPS.flatMap((group) =>
    group.values.map((value) => ({ label: t(`diaryFilters.sortOptions.${value}`), value }))
  )
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button className="md:hidden" size="sm" variant="secondary">
            ...
            {activeCount > 0 ? (
              <span aria-label={t('diaryFilters.activeCount', { count: activeCount })}>
                {activeCount}
              </span>
            ) : null}
          </Button>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          variant={value === 'any' ? 'secondary' : 'outline'}
        >
          <span className="truncate">{value === 'any' ? label : selected.label}</span>
          <ChevronDown data-icon="inline-end" />
        </Button>
      </DropdownMenuTrigger>
