...
export function HeatmapCalendar({
  ...
  const monthLabels = React.useMemo(() => {
    ...
    for (const [i, col] of columns.entries()) {
      const firstInCol = col.find((cell) => !cell.disabled)?.date ?? col.at(0)?.date

      if (!firstInCol) continue

      const prevCol = i > 0 ? columns.at(i - 1) : undefined

      const prevFirst = prevCol?.find((cell) => !cell.disabled)?.date ?? prevCol?.at(0)?.date
