    ...
    <KinoFrame label="Discover · Track · Share">
      ...
      >
        ...
        >
          ...
          <SupportingText>
            Discover movies and series, track episodes, rate titles, and build watchlists with
            friends.
          </SupportingText>
      ...
      >
        ...
        >
          ...
          <SupportingText maxWidth={610}>
            Find trending stories, new releases, and acclaimed movies and series worth keeping.
          </SupportingText>
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
    ...
    >
      <div
        style={{
          ...
          display: 'flex',
          borderRadius: 28,
          background: 'radial-gradient(circle at 58% 48%, rgba(29,185,84,0.13), transparent 64%)',
        }}
      />
      {lists.map((list, listIndex) => (
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', color: colors.muted, fontSize: 15 }}>{list.count}</div>
            {list.collaborators ? (
              <div style={{ display: 'flex', alignItems: 'center' }}>
                {['#b9f6ce', '#99a89f', '#65736b'].map((background, avatarIndex) => (
                  <div
                    key={background}
                    style={{
                      width: 24,
          ...
          <div
            style={{
              ...
              justifyContent: 'center',
              borderRadius: 999,
              background: list.accent ? colors.accent : 'rgba(255,255,255,0.07)',
        ...
        >
          ...
          <SupportingText maxWidth={650}>
            {data.titles.length > 2
              ? `Explore a curated collection including ${data.titles
                  .slice(0, 3)
                  .map((item) => item.title)
                  .join(', ')}, and many more.`
              : data.titles.length > 0
                ? `Featuring ${data.titles.map((item) => item.title).join(' and ')}.`
                : data.description || 'A public collection ready for its first title.'}
          </SupportingText>
          <div style={{ display: 'flex', alignItems: 'center', marginTop: 18 }}>
            ...
            >
              {data.owner.username ? `@${data.owner.username}` : data.owner.displayName}
            </div>
          </div>
        </div>
      </div>
...
function WatchlistBackground({ images }: { images: Array<OgImage | null> }) {
  const availableImages = images.filter((image): image is OgImage => Boolean(image))
