...
export const getTitleSeoData = cache(
  async (tmdbId: number, type: MediaType, language = "en") => {
...
export const getTitleSeoDataById = cache(
  async (tmdbId: number, language = "en") => {
    try {
...
export const getTitleSeoDataBySegment = cache(
  async (tmdbId: number, slug: string, language = "en") => {
    const movie = await getTitleSeoData(tmdbId, "movie", language).catch(
      () => null,
    );
    if (movie && (!slug || slugify(movie.title) === slug)) return movie;

    const series = await getTitleSeoData(tmdbId, "tv", language).catch(
      () => null,
    );
...
export const getPersonSeoData = cache(
  async (personId: number, language = "en") => {
    const tmdb = createTmdb(language);
    return tmdb.getPersonDetails(personId);
  },
);

export function getPersonVisuals(
  person: Awaited<ReturnType<typeof getPersonSeoData>>,
) {
...
const DISCOVER_CACHE_SECONDS = 60 * 60 * 24 * 7;
const DISCOVER_LIMIT = 6;

async function fetchDiscoverData(language: string) {
  const tmdb = createTmdb(language);
  ...
  const trending = await enrichTitlesWithPalette(
    trendingTitles.slice(0, DISCOVER_LIMIT),
  );
