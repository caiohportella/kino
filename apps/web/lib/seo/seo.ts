import type { TitleDetails, TMDbPerson } from '@kino/core'
...
export function getTitlePresentation(details: Pick<TitleDetails, 'title' | 'year'>) {
  const title = details.title.replace(/\s+/g, ' ').trim()
  const year = Number.isInteger(details.year) && details.year >= 1888 ? details.year : null
...
export function buildPersonSchema({
  ...
  person: Pick<
    TMDbPerson,
    'name' | 'biography' | 'birthday' | 'deathday' | 'place_of_birth' | 'known_for_department'
  >
  url: string
  image: string | null
}) {
