...
import type { EpisodeRating, TMDbEpisode, TMDbSeason } from '~/types'
...
export function SeasonSection({
  ...
  const [seasons, setSeasons] = useState<Pick<TMDbSeason, 'id' | 'season_number' | 'name'>[]>([])
  ...
  const handleRateSeason = async (rating: number) => {
    ...
    setLoading(true)
    const episodesToRate = episodes.filter((ep) => !ratings[ep.episode_number]?.rating)
    ...
    async (seasonNum: number) => {
      ...
      try {
        ...
        if (isAuthenticated) {
          ...
          const userRatings = await dbService.getUserSeasonRatings(titleId, seasonNum)
  ...
  const handleUnwatchEpisode = async (episodeNumber: number) => {
    if (selectedSeason === null) return
    try {
      await dbService.removeEpisodeRating(titleId, selectedSeason, episodeNumber)
      await loadSeasonDetails(selectedSeason)
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error)
  ...
  return (
    <View className="mt-6">
      <Text className="mb-4 text-xl font-bold text-text-primary">{t('seasons.title')}</Text>
        ...
        {seasons.map((season) => (
          <TouchableOpacity
            ...
            className={`mr-3 rounded-full px-4 py-2 ${
              selectedSeason === season.season_number ? 'bg-accent' : 'bg-surface'
            }`}
            onPress={() => setSelectedSeason(season.season_number)}
          >
            <Text
              className={`font-semibold ${
                selectedSeason === season.season_number ? 'text-primary' : 'text-text-secondary'
              }`}
            >
      ...
      {episodes.length > 0 && (
        <TouchableOpacity
          disabled={false}
          className={`mb-4 flex-row items-center justify-center rounded-lg py-3 ${
          ...
          onPress={() => {
            ...
            Alert.alert(
              isWatched ? t('seasons.markSeasonUnwatched') : t('seasons.markSeasonWatched'),
              isWatched
              ...
              [
                { text: t('common.cancel'), style: 'cancel' },
                {
                  ...
                  onPress: async () => {
                    ...
                    try {
                      if (isWatched) {
                        await dbService.removeSeasonEpisodesWatched(titleId, selectedSeason)
                      } else {
                      ...
                      Alert.alert(
                        ...
                        t('seasons.markSeasonError', {
                          status: isWatched ? t('seasons.notWatched') : t('seasons.watched'),
                        })
                      )
                    } finally {
                      setLoading(false)
        ...
        >
          <Ionicons
            ...
            size={20}
            color={seasonStats?.ratedEpisodes === episodes.length ? 'white' : '#1DB954'}
          />
          <Text
            className={`ml-2 font-semibold ${
              seasonStats?.ratedEpisodes === episodes.length ? 'text-white' : 'text-[#1DB954]'
            }`}
          >
            {seasonStats?.ratedEpisodes === episodes.length
              ? t('seasons.watched')
      ...
      {loading ? (
        ...
        seasonStats.ratedEpisodes > 0 && (
          <View className="mb-4 flex-row items-center justify-between rounded-lg border border-black/50 bg-surface p-4">
            <View>
              <Text className="text-text-secondary">{t('seasons.seasonScore')}</Text>
              {seasonStats.averageRating > 0 ? (
                <Text className="text-2xl font-bold text-text-primary">
                  {seasonStats.averageRating.toFixed(1)}{' '}
                  <Text className="text-sm font-normal text-text-secondary">/ 5</Text>
                </Text>
              ) : (
                <TouchableOpacity
                  onPress={() => {
            ...
            <View>
              <Text className="text-text-secondary">{t('seasons.progress')}</Text>
      ...
      {loading ? (
        <View className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <View key={i} className="h-24 w-full animate-pulse rounded-lg bg-surface/50" />
          ))}
        </View>
      ) : (
        <View>
              ...
              <View key={ep.id} className="mb-4 border-b border-surface pb-4">
                <View className="flex-row items-start justify-between">
                  <View className="flex-1 pr-4">
                    ...
                    <Text className="leading-5 text-text-secondary" numberOfLines={3}>
                      {ep.overview || t('seasons.noOverview')}
                    </Text>
                    {isWatched && userRating.watchedAt && (
                      <Text className="mt-2 text-xs text-accent">
                        {t('seasons.watchedOn')} {formatDate(userRating.watchedAt)}
                      </Text>
                    )}
                  </View>
                  {hasAired ? (
      ...
      {selectedEpisodeForAction && selectedSeason !== null && (
        <EpisodeActionModal
          ...
          onRate={(rating, watchType) =>
            handleRateEpisode(selectedEpisodeForAction.episode_number, rating, watchType)
          }
        />
      )}
