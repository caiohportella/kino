...
import { getOscarNominationsLegacy, isOscarNomineeLegacy, useOscarData } from '~/services/awards'
...
export function OscarBadge({ tmdbId, showLabel = true, size = 'normal' }: OscarBadgeProps) {
  ...
  const nominations = awards
    ? awards[tmdbId]
    : isOscarNomineeLegacy(tmdbId)
      ? getOscarNominationsLegacy(tmdbId)
      : null
  ...
  return (
    <View className="absolute top-2 left-0 right-0 items-center z-10">
      ...
      >
        ...
        {showLabel && (
          <Text className="text-white font-black uppercase tracking-widest text-[8px]">
