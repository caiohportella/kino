...
const STUB_OSCAR_2026_DATA: Record<number, OscarNomination[]> = {
  ...
  1525091: [
    // All the Empty Rooms', isWinner: false }
...
export function useOscarData(year: number = 2026) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const mergedNominations: Record<number, OscarNomination[]> = JSON.parse(
        JSON.stringify(STUB_OSCAR_2026_DATA)
      )
      ...
      data.forEach((row) => {
        ...
        if (existingIdx >= 0) {
          ...
          mergedNominations[row.tmdb_id][existingIdx].isWinner =
            row.is_winner || mergedNominations[row.tmdb_id][existingIdx].isWinner
        } else {
