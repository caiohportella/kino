import type { ImportMoviePayload, ImportTitleItem, ParsedImportResult } from '~/types/imports'
...
export async function parseImportFileFromUri(
  ...
  _readBase64?: (uri: string) => Promise<string>
): Promise<ParsedImportResult> {
  if (fileName.toLowerCase().endsWith('.csv')) {
    return parseLetterboxdCsv(await readText(uri), fileName)
  }
  return {
    source: null,
    fileName,
...
export function parseLetterboxdCsv(
  ...
  for (const record of records) {
    ...
    const rewatch = parseBooleanLike(firstValue(record, ['Rewatch', 'rewatch', 'Rewatch?']))
  ...
  return {
    ...
    items,
    warnings,
    errors: items.length ? [] : ['No Letterboxd rows could be parsed.'],
  }
}
