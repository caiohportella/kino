...
export type VisibilityScope =
  { kind: "public" } | { kind: "authenticated"; userId: string };
...
export const titleQueryKeys = {
  ...
  summary: (input: TitleQueryInput) =>
    createTitleQueryDescriptor(input).summary,
  detailsRoot: () => [...TITLE_ROOT, "details"] as const,
  details: (input: TitleQueryInput) =>
    createTitleQueryDescriptor(input).details,
...
export const profileQueryKeys = {
  ...
  watchedMovies: (input: ProfileSectionQueryInput) =>
    profileSectionKey("watched-movies", input),
  watchedMoviesRoot: (input: ProfileIdentityQueryInput) =>
    profileSectionRootKey("watched-movies", input),
  watchedSeries: (input: ProfileSectionQueryInput) =>
    profileSectionKey("watched-series", input),
  watchedSeriesRoot: (input: ProfileIdentityQueryInput) =>
    profileSectionRootKey("watched-series", input),
  diaryEntries: (input: ProfileSectionQueryInput) =>
    profileSectionKey("diary-entries", input),
  ...
  watchlists: (input: ProfileSectionQueryInput) =>
    profileSectionKey("watchlists", input),
  watchlistsRoot: (input: ProfileIdentityQueryInput) =>
    profileSectionRootKey("watchlists", input),
  reviews: (input: ProfileSectionQueryInput) =>
    profileSectionKey("reviews", input),
  reviewsRoot: (input: ProfileIdentityQueryInput) =>
    profileSectionRootKey("reviews", input),
  ratings: (input: ProfileSectionQueryInput) =>
    profileSectionKey("ratings", input),
  ratingsRoot: (input: ProfileIdentityQueryInput) =>
    profileSectionRootKey("ratings", input),
...
function profileSectionRootKey(
  section: string,
  input: ProfileIdentityQueryInput,
) {
  return [
    ...PROFILE_ROOT,
    section,
...
function createTitleQueryDescriptor(
  input: TitleQueryInput,
): TitleQueryDescriptor {
...
function scopeSegments(scope: CacheScope) {
  ...
  return [
    "authenticated",
    requireIdentifier(scope.userId, "authenticated user id"),
  ] as const;
}
...
function normalizeFilterRecord(
  filters: Readonly<Record<string, CacheFilterValue>>,
): CacheFilters {
  return Object.fromEntries(
    Object.keys(filters)
      ...
      .map((key) => [
        key,
        normalizeFilterValue(filters[key] as CacheFilterValue),
      ]),
  );
}
...
function normalizeFilterValue(value: CacheFilterValue): CacheFilterValue {
  ...
  if (value !== null && typeof value === "object")
    return normalizeFilterRecord(value);
  if (
    value === null ||
  ...
  throw new TypeError(
    "Cache filters must contain only serializable finite values.",
  );
}

function isFilterArray(
  value: CacheFilterValue,
): value is readonly CacheFilterValue[] {
