...
function movieDiaryRow(titleId, watchedAt, runtime, watchType = 'first-time', metadata = {}) {
  return {
    title_id: titleId,
    watched_at: watchedAt,
...
function makeSharedRecapRows() {
  const movieA = movieDiaryRow('runner-up', '2026-01-02T12:00:00.000Z', 90, 'first-time', {
    title: 'Runner Up',
    tmdb_id: 101,
  })
  const movieB = movieDiaryRow('winner', '2026-01-03T12:00:00.000Z', 150, 'first-time', {
    title: 'Winner',
    tmdb_id: 102,
  })
  const movieC = movieDiaryRow('third', '2026-01-04T12:00:00.000Z', 120, 'first-time', {
    title: 'Third',
    tmdb_id: 103,
  })
...
test('Bayesian confidence favors repeated strong work over a low-sample perfect result', () => {
  const stats = calculateProfileRatingStats([
    ...['one', 'two', 'three'].map((id, index) =>
      ratedMovieRow(id, 5, `2026-02-${String(index + 1).padStart(2, '0')}T00:00:00.000Z`, {
        cast: [person('One', 10)],
      })
    ),
    ...['four', 'five', 'six', 'seven', 'eight', 'nine', 'ten'].map((id, index) =>
      ratedMovieRow(id, 4.8, `2026-02-${String(index + 4).padStart(2, '0')}T00:00:00.000Z`, {
        cast: [person('Consistent', 10)],
      })
    ),
    ...['eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen'].map((id, index) =>
      ratedMovieRow(id, 3, `2026-03-${String(index + 1).padStart(2, '0')}T00:00:00.000Z`)
    ),
  ])
...
test('calculates half-star rating buckets and null average for zero ratings', () => {
  const stats = calculateProfileRatingStats([{ rating: 5 }, { rating: 4.5 }, { rating: null }])

  assert.equal(stats.totalRatings, 2)
  assert.equal(stats.averageRating, 4.8)
  assert.equal(stats.distribution.find((bucket) => bucket.rating === 4.5)?.count, 1)
    ...
    diaryRows: [
      {
        ...
        titles: {
          ...
          type: 'movie',
          genres: [],
          production_companies: [{ id: 1, name: 'Studio One', logo_path: '/studio-one.png' }],
          runtime: 120,
      ...
      },
      {
        ...
        titles: {
          ...
          production_companies: [],
          tmdb_data: {
            production_companies: [{ id: 2, name: 'Studio Two', logo_path: '/studio-two.png' }],
          },
          runtime: 90,
          episode_runtime: null,
        },
  ...
  const recap = buildProfileMonthlyRecap({
    ...
    current: {
      diaryRows: [
        ...
        movieDiaryRow('mixed-movie', '2026-08-12T12:00:00.000Z', 120, 'rewatch'),
      ],
      movieRatingRows: [],
      episodeRatingRows: [
...
test('monthly highest-rated categories use only the supplied current-month rows', () => {
  ...
  const currentMovieB = ratedMovieRow('aug-b', 4.5, '2026-08-03T00:00:00.000Z', {
    genres: [{ id: 1, name: 'Drama' }],
  })
...
test('month-bounded recaps ignore out-of-range rating rows', () => {
  const inMonthWatch = movieDiaryRow('aug-watch', '2026-08-02T12:00:00.000Z', 100, 'first-time', {
    title: 'August Watch',
    genres: [{ id: 1, name: 'Drama' }],
  })
  const inMonthRatingA = ratedMovieRow('aug-a', 4, '2026-08-03T00:00:00.000Z', {
    title: 'August A',
    genres: [{ id: 1, name: 'Drama' }],
  })
  const inMonthRatingB = ratedMovieRow('aug-b', 3, '2026-08-04T00:00:00.000Z', {
    title: 'August B',
    genres: [{ id: 1, name: 'Drama' }],
  })
  const outOfMonthRating = ratedMovieRow('aug-watch', 5, '2026-07-31T00:00:00.000Z', {
    title: 'August Watch',
    genres: [{ id: 2, name: 'Comedy' }],
  })
