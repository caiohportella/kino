...
export function selectLocalizedImage(
  input: SelectLocalizedImageInput,
): LocalizedImageSelection {
  ...
  const candidates = input.candidates
    .filter(
      (
        candidate,
      ): candidate is LocalizedImageCandidate & { filePath: string } =>
  ...
  const tiers: readonly ImageTier[] = [
    {
      ...
      matches: (language, region) =>
        (language === locale &&
          (region === null || region === requestedRegion)) ||
...
function normalizeCandidateLocale(
  locale: string | null,
...
function normalizeCandidateRegion(
  region: string | null | undefined,
): string | null | undefined {
...
function compareQuality(
  ...
  const preferredAspectRatio =
    kind === "backdrop" || kind === "logo" ? 16 / 9 : 2 / 3;
  ...
  const qualityOrder = compareDescending(
    finiteOrZero(left.quality),
    finiteOrZero(right.quality),
  );
  if (qualityOrder !== 0) return qualityOrder;

  const resolutionOrder = compareOptionalDescending(
    pixelArea(left),
    pixelArea(right),
  );
