...
test("builds watchlist picker summaries with counts and recent unique covers in one query", async () => {
  ...
  const summaries = await service.getWatchlistPickerSummaries([
    "list-1",
    "list-2",
    "list-3",
  ]);
  ...
  assert.equal(
    calls.filter(
      (call) => call.kind === "from" && call.table === "watchlist_items",
    ).length,
    1,
  );
...
test("gets title contributors for known watchlists without reloading watchlists", async () => {
  ...
  const contributors =
    await db.getWatchlistTitleContributorsForWatchlists(
      "title-1",
      ["list-1", "list-2"],
    );
...
test("builds picker summaries and title contributors from one watchlist-items query", async () => {
  ...
  const picker = await db.getWatchlistPickerData(
    ["list-1", "list-2"],
    "title-1",
  );

  assert.equal(
    calls.filter(([type]) => type === "from").length,
    1,
  );
  ...
  assert.deepEqual(
    picker.summaries["list-1"].coverItems,
    [
      {
        tmdbId: 202,
        type: "movie",
